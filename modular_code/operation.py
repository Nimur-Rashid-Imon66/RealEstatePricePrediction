...
def inputform():
    st.markdown("<h1 style='text-align: left;margin-top:-2rem; margin-left:1rem; color: #E12D06;'>Real Estate Price Prediction</h1>", unsafe_allow_html=True)
    st.write('\n')
    ...
    with st.form(key='submit_form', clear_on_submit=False, border=True):
            ...
            if st.form_submit_button(label='Submit'):
                    if not(PropertyType and ClubHouse and School_University_in_Township):
                        ...
                        lowerlimit = prediction - 5
                        upperlimit = prediction + 5
